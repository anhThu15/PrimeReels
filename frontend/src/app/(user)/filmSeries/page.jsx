...
export default function FilmSeries() {
    ...
    const fetchGenresAndMovies = async () => {
      try {
        ...
        const [resGenres, resMovies] = await Promise.all([
          ...
          axios.get(`/api/movies-type/1`) // Lấy danh sách phim
        ]);

        const bannerData = resMovies.data.slice(0, 3); // Chỉ lấy 3 phần tử đầu tiên
        ...
        const resCountry = await axios.get(`/api/movie-types/1/country/Việt Nam`);
        ...
        for (let genre of genresData) {
          genresMovies[genre.genre_id] = moviesData.filter(movie => movie.genre_id === genre.genre_id);
        }
  ...
  const handleGenreChange = (event) => {
    ...
    if (selectedGenreId) {
      router.push(`/filterFilmSeries?genreId=${selectedGenreId}&movieTypeId=1`); // Điều hướng đến trang lọc phim theo thể loại
    }
  };
        ...
        <div className="container-fluid p-0">
          ...
          <div>
            {Object.keys(moviesData.moviesByGenre).map((genreId) => (
              moviesData.moviesByGenre[genreId].length > 0 && ( // Chỉ hiển thị nếu có phim trong thể loại này
                <div key={genreId}>
