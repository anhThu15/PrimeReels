...
export default function Comment(props) {
  ...
  // Cập nhật giá trị rating nếu người dùng đã bình luận
  useEffect(() => {
    if (user) {
      const userComment = comments.find(comment => comment.user_id === user?.user_id);
      if (userComment) {
        setValue('rating', userComment?.rating); // Cập nhật rating cho người dùng nếu có bình luận
      } else {
        setValue('rating', 0);  // Nếu chưa có bình luận, để rating mặc định là 0
      }
    }
  }, [user, comments, setValue]);
